...
/**
 * Performance test for the AccountDetails entity.
 */
class AccountDetailsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the AccountDetails entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all accountDetails")
            .get("/api/account-details")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new accountDetails")
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_accountDetails_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created accountDetails")
                .get("${new_accountDetails_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created accountDetails")
            .delete("${new_accountDetails_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
