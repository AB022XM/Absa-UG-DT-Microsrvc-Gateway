...
/**
 * Performance test for the DefaultSettings entity.
 */
class DefaultSettingsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the DefaultSettings entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all defaultSettings")
            .get("/api/default-settings")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new defaultSettings")
            ...
            .body(StringBody("""{
                ...
                , "token":"SAMPLE_TEXT"
                , "thirdPartyDTransactionId":"SAMPLE_TEXT"
                , "defaultSettingCode":"SAMPLE_TEXT"
                , "jsonSettings":null
                , "appConfig":null
                , "appName":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_defaultSettings_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created defaultSettings")
                .get("${new_defaultSettings_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created defaultSettings")
            .delete("${new_defaultSettings_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
