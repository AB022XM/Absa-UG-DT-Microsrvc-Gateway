...
/**
 * Performance test for the PaymentConfig entity.
 */
class PaymentConfigGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the PaymentConfig entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all paymentConfigs")
            .get("/api/payment-configs")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new paymentConfig")
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_paymentConfig_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created paymentConfig")
                .get("${new_paymentConfig_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created paymentConfig")
            .delete("${new_paymentConfig_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
