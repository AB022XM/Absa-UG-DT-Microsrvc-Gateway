...
/**
 * Performance test for the DTransactionHistory entity.
 */
class DTransactionHistoryGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the DTransactionHistory entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all dTransactionHistories")
            .get("/api/d-transaction-histories")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new dTransactionHistory")
            .post("/api/d-transaction-histories")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_dTransactionHistory_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created dTransactionHistory")
                .get("${new_dTransactionHistory_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created dTransactionHistory")
            .delete("${new_dTransactionHistory_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
