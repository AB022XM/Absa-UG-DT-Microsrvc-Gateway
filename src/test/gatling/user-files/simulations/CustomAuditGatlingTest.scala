...
/**
 * Performance test for the CustomAudit entity.
 */
class CustomAuditGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the CustomAudit entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all customAudits")
            .get("/api/custom-audits")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new customAudit")
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_customAudit_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created customAudit")
                .get("${new_customAudit_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created customAudit")
            .delete("${new_customAudit_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
